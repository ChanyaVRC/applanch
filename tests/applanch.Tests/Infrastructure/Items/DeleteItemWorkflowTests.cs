...
public class DeleteItemWorkflowTests
{
    ...
    public void TryDelete_ConfirmEnabledAndCancelled_ReturnsCancelledAndDoesNotRemove()
    {
        ...
        {
            new(new LaunchPath(@"C:\Tools\app.exe"), Category.FromInput("Dev"), string.Empty, "App")
        };
        var item = items[0];
        var removed = false;
    ...
    public void TryDelete_Success_ReturnsOriginalIndexAndRemovesItem()
    {
        ...
        var items = new List<LaunchItemViewModel>
        {
            new(new LaunchPath(@"C:\Tools\a.exe"), Category.FromInput("Dev"), string.Empty, "A"),
            new(new LaunchPath(@"C:\Tools\b.exe"), Category.FromInput("Dev"), string.Empty, "B"),
            new(new LaunchPath(@"C:\Tools\c.exe"), Category.FromInput("Dev"), string.Empty, "C")
        };
        var target = items[1];
    ...
    public void TryDelete_ItemNotInList_ReturnsMinusOneIndex()
    {
        ...
        var existing = new LaunchItemViewModel(new applanch.Infrastructure.Utilities.LaunchPath(@"C:\Tools\a.exe"), Category.FromInput("Dev"), string.Empty, "A");
        var target = new LaunchItemViewModel(new applanch.Infrastructure.Utilities.LaunchPath(@"C:\Tools\x.exe"), Category.FromInput("Dev"), string.Empty, "X");
